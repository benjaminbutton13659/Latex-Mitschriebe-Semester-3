...
\begin{tikzpicture}
	...
	\draw ($ (o) + (0,-1) $) -- ++(0,2) node[above] {$ \sigma $};
	\draw ($ (o) + (1.5,-1) $) -- ++(0,2) node[above] {$ -\sigma $};
...
\begin{minipage}{.5\linewidth}
	...
	\begin{tikzpicture}
		...
		\draw ($ (y) + (0,-.2) $) -- (0,1.5) node[anchor=south east] {$ y $};
		\draw ($ (x) + (-.2,0) $) -- (2,0) node[anchor=north west] {$ x $};
		\draw ($ (y) + (0,-.2) $) -- ++(.2,.2);
	\end{tikzpicture}
\end{minipage}
\begin{minipage}{.5\linewidth}
...
\begin{enumerate}[1)]
	...
	\begin{equation*}
	g(x) = \sum_{n} an \sqrt{\frac{l}{2}} \sin \left(\frac{n \pi x}{l}\right)
	\end{equation*}
...
\begin{enumerate}[i)]
	\item $$\frac{\textrm{d}^2 Q}{\textrm{d} \varphi^2} + m^2 Q = 0$$ Lösung: $$Q(\varphi) = e^{im\varphi} = \cos(m\varphi) + i\sin(m\varphi)$$
	 	...
	 	$$\frac{r^2}{U} \frac{\textrm{d}^2 U}{\textrm{d} r^2} = - \frac{1}{P\sin\theta}\frac{\textrm{d} P}{\textrm{d} \theta}\left( \sin\theta \frac{\textrm{d}P}{\textrm{d}\theta} \right)$$ % montegue
...
\end{enumerate}
\frbox{Zugeordnete Legendresche DGL}{$$\Rightarrow \frac{\textrm{d}}{\textrm{d}x} \left( (1-x^2) \frac{\textrm{d}P}{\textrm{d}x} \right) + \left( \lambda - \frac{m^2}{1 - x^2} \right) P(x) = 0$$} % T2
