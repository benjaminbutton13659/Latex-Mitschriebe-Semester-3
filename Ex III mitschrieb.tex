...
\end{itemize}
\textbf{(nutzen) senkrechten Einfall}\\
$ \alpha,\beta \approx 0 $\\
...
allgemein:
\begin{equation*}
\vec{E}(\vec{x},t) = \int \vec{p}(\varphi) \vec{E}(\vec{x},t,p) \dd p
\end{equation*}
...
\subsection{Wellengleichung in Materia}
...
\subsection{Lösung für kleine imaginäre Anteile.}
...
In Materie gibt es transversale EM-Wellen mit $ c = \frac{c_0}{\sqrt{\epsilon'}} $
...
\textbf{Theorie:}\\
Prisma: $ \hat{\epsilon} = \epsilon' + i \epsilon'' $\\
