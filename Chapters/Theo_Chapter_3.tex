...
\begin{minipage}{.55\linewidth}
	...
	%t1:
	\begin{tikzpicture}[scale=.8]
		\draw[thick,fill=black!10!red!12!white, domain=-3.75:3.75, samples=500] plot (\x, {2 * cos(1 / 4 * 15 * \x r) * exp(- (\x)^(2) * 0.25});
...
\begin{minipage}{.5\linewidth}
	...
	\begin{tikzpicture}[scale=.7]
		...
		\draw[thick,domain=0.001:9.3, samples=500] plot (\x, {2 * cos(1 / 4 * 15 * (\x * .5) r) * exp(- (\x * .5)^(2) * 0.25 * (1) / (\x * 0.2) });
...
%\begin{tikzpicture}
%\draw [domain=-4:4, samples=500,scale=.3] plot (\x, {5*exp(-(\x)^(2)*0.25 * cos(\x * 40)});
%\end{tikzpicture}
...
Realteil:
$$ \Re(\epsilon_r(\omega)) = 1 + \frac{Ne^2}{\epsilon_m} \frac{\omega_0^2 - \omega^2}{(\omega_0^2 - \omega^2) + \gamma^2 \omega^2} $$
Imaginärteil:
$$ \Im(\epsilon_r(\omega)) = \frac{N e^2}{\epsilon_0 m} \frac{\gamma \omega}{(\omega_0^2 - \omega^2) + \gamma^2 \omega^2} \quad \ \  $$
%
%
%
% T5
...
\textbf{Verallgemeinert auf U Elektronen im Atom:}\\
...
\subsection{Bedeutung der komplexen/frequenzabhängigen Dielektrizitätskonstante}
...
\end{equation*}
\begin{equation*}
\vec{E}(\vec{r},t) = \vec{E}_0 e^{- k_I z} e^{i(k_R z - \omega t)}
\end{equation*}
%
%
%
...
\section{Allgemeine Lösung der Maxwell-Gl. - Retardierte Potentialen}
...
\end{equation*}
Aus den inhomogenen Gleichungen erhalten wir:
\begin{equation*}
\Delta \Phi - \frac{1}{c^2} \prt{^2 \Phi}{t^2} = - \frac{1}{\epsilon_0} \rho
\end{equation*}
\begin{equation*}
\Delta \vec{A} - \frac{1}{c^2} \prt{^2 \vec{A}}{t^2} = - \mu \vec{j}
\end{equation*}
\emph{Nebenbemerkung:}
\begin{equation*}
...
\overset{\substack{\tx{Fourriertrafo.} \\ \tx{von } \rho}}{=} & \frac{1}{\sqrt{1 \pi}} \int \dd \omega e^{- i \omega t} \left(- \frac{1}{\epsilon_0}\right) \tilde{\Phi} (\vec{r},\omega)
\end{align*}
...
&= \kq \int \dd^3 r' \frac{1}{|\vec{r} - \vec{r}'|} \ub{\frac{1}{\sqrt{2 \pi}} \int \dd \omega \tilde{\rho} (\vec{r}', \omega) e^{- i \omega (t - \frac{|\vec{r} - \vec{r}'|}{c} )}}_{= \rho(\vec{r}', t - \frac{1}{c} |\vec{r} - \vec{r}'|)}
\end{align*}
\begin{equation*}
