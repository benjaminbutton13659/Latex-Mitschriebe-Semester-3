...
\begin{enumerate}[I)]
	...
	\end{minipage}%
	\\
	Aus den Stetigkeitsbedingunen folgt:  % vielleicht muss eins auch i) sein -- nein muss es nicht:
	\begin{equation*}
...
\begin{minipage}{.5\linewidth}
	...
	\lcom{Dies wird z.B. im Griffitz genau hergeleitet, ist aber obwohl es nicht zu kompliziert ist, zu langwierig für die Vorlesung.}
\end{minipage}%
\begin{minipage}{.5\linewidth}
	\flushright
	% t6:
...
\begin{minipage}{.5\linewidth}
	$$ \vec{d}_{\tx{ret}} \defeq \vec{r} - \vec{R}(t_{\tx{ret}}) \qquad \vec{e}_{\tx{ret}} \defeq \frac{\vec{d}_{\tx{ret}}}{|\vec{d}_{\tx{ret}}|} $$
...
\begin{center}
	...
	\begin{tikzpicture}[scale=0.8]
		\draw[rounded corners=5pt, thick] (0.15,0) -- ++(0,.5) -- ++(2.5,0);
		\draw[rounded corners=5pt, thick] (-0.15,0) -- ++(0,.5) -- ++(-2.5,0);
		\node[below] at (0,0) {$ \tilde{U} $};
		\coordinate (a) at (2,0);
		\draw[<->] ($ (0,.8) - (a) $) -- node[above] {$ e^- $} ($ (0,.8) + (a) $);
	\end{tikzpicture}
	\hspace{2.5cm}
	\begin{tikzpicture}[scale=0.8]
		\draw[fill = white!20!red] (0,0) circle (.2cm) node[] {$ \color{white} + \color{white} $};
		\draw (0,0) circle (1.1cm);
		\draw[fill = white!20!blue] (45:1.1cm) circle (.2cm) node[] {$ \color{white} - \color{white} $};
	\end{tikzpicture}
\end{center}
Vektorpotential:
\begin{align*}
...
\end{align*}
\begin{minipage}{.6\linewidth}
	Wir interessieren uns für zeitabhängige Felder in Raumbereichen außerhalb der Ladung-/Strom\-verteilung.
\end{minipage}%
\begin{minipage}{.4\linewidth}
	\flushright
	% t2:
	\begin{tikzpicture}
		...
		\draw[thick, ->] (0,0) -- node[above] {$ R_0 $} (150:1.5cm);
		\draw[thick, ->] (0,0) -- (40:2cm) node[anchor=south west] {$ \vec{r} $};
		\draw[thick, ->] (0,0) -- (70:0.7cm) node[above] {$ \vec{r}' $};
		\node at (-.25,-.3) {$ \rho $};
		\node at (.4,-.7) {$ \vec{j} $};
	\end{tikzpicture}
\end{minipage}%
...
\begin{minipage}{.8\linewidth}
	\begin{equation*}
	\Rightarrow \quad \vec{B}(\vec{r},t) = \frac{\mu_0}{4 \pi} ck^2 \frac{e^{i(kr - \omega t)}}{r} \left(1 - \frac{1}{ikr}\right) \vec{e}_r \times \vec{p} \qquad \qquad \vec{B} \perp \vec{e}_r , \vec{r}
	\end{equation*}
\end{minipage}%
\begin{minipage}{.2\linewidth}
	\flushright
	% t3:
	\begin{tikzpicture}[scale=0.8]
		\draw[very thick, ->] (0,-1) -- (0,1) node[left] {$ \vec{p} $};
		\draw[->] ({0.8 * sin(45)},-{.4 * cos(45)}) arc[start angle=-45, end angle=315, x radius=0.8, y radius=.4];
...
\begin{minipage}{.5\linewidth}
	Für den Fernbereich gilt:\\
	$ kr \gg 1 \quad r \gg \lambda $\\
	Damit erhalten wir die Näherungen\\
	$ \frac{1}{kr} \ll 1 $\\[5pt]
	$ \frac{k^2}{r} = \frac{k^2 r^2}{r^3} \gg \frac{1}{r^3} $\\[5pt]
	$ \frac{k}{r^2} = \frac{k^2}{kr r} \ll \frac{k^2}{r} $
\end{minipage}%
\begin{minipage}{.5\linewidth}
	\flushright
	% t4:
	\begin{tikzpicture}[scale=.8]
		...
		\node at (-.25,-.3) {$ \rho $};
		\node at (.4,-.7) {$ \vec{j} $};
		\draw[<->] (0,-2) -- node[below] {$ R_0 $} ++(1.5,0);
		\draw[<->] (0,-2.75) -- node[below] {$ \lambda $} ++(3,0);
		\draw[<->] (0,-3.5) -- node[below] {$ \vec{r} $} ++(6,0);
	\end{tikzpicture}
\end{minipage}%
\begin{equation*}
...
%
%
% I33 glaube oben muss ein k^2 bei E hin aber da stand keins ?
%
%
%
...
\begin{minipage}{.6\linewidth}
	...
	\begin{tikzpicture}
		...
		\centerarc[thick, draw=lightgray](0,0)(25:-25:\x);
		\draw[->] (0,0) -- (0,2) node[anchor=south east] {$ z $};
		\draw[->] (0,0) -- (1.5,1.5) node[anchor=south east] {$ y $};
		\draw[->] (0,0) -- (8,0) node[anchor=north west] {$ x $};
		\draw[very thick, ->] (0,0) -- node[left] {$ \vec{p} $} (0,1);	\coordinate (o) at (3.5,0);
		\draw[very thick, ->] (0,0) -- node[below] {$ \vec{r} $} (o);
		\draw[very thick, ->] (o) -- ++(0,1) node[anchor=south east] {$ \vec{E} $};
		\draw[very thick, ->] (o) -- ++(-0.75,-0.75) node[anchor=south east] {$ \vec{B} $};
		\draw[very thick, ->] (o) -- ++(1,0) node[anchor=north west] {$ \vec{e}_r $};
	\end{tikzpicture}
\end{minipage}%
\\
...
\begin{minipage}{.3\linewidth}
	...
	% t2:
	\begin{tikzpicture}
		\draw[thick, ->] (0,0) -- node[left] {$ \vec{p} $} ++(0,2);
		\draw[thick, ->] (0,0) -- node[below] {$ \vec{r} $} ++(30:2);
		\centerarc[](0,0)(30:90:1.2);
		\node at (60:0.8) {$ \theta $};
	\end{tikzpicture}
\end{minipage}%
\\
...
\begin{enumerate}[i)]
	...
	\item 
	\begin{minipage}{.6\linewidth}
			in Raumwinkelelementen $ \dd \Omega $ abgestrahlte Leistung
			\begin{equation*}
	...
	\begin{minipage}{.4\linewidth}
		...
		\begin{tikzpicture}
			...
			\centerarc[](0,0)(30:90:1.2);
			\node at (60:0.8) {$ \theta $};
			\draw[rotate around={-20:(30:2)}] ($ (30:2) + (-.5,-.5) $) -- ++(0,1) -- ++(1,0) -- ++(0,-1) -- cycle;
			\draw[shift=(30:2), rotate=-20, pattern = north east lines] (-.5,-.5) -- ++(0,1) -- ++(1,0) -- ++(0,-1) -- cycle;
			% rotate around={45:(0,0)}
			% shift=(30:2), rotate=-20
			\draw (25:2) -- ++(0.5,0) node[right] {$ \dd \vec{f} = \vec{e}_r  r^2 \dd \Omega $};
		\end{tikzpicture}
		\vspace{5pt}
	\end{minipage}%
	\\
	...
	\begin{center}
		% t4:
		\begin{tikzpicture}[scale=.8]
			\draw[looseness=1, ultra thick, draw=black!30!white] (0,0) to[out=55,in=180] (1.5,1.25) to[out=0,in=90] (2.5,0) to[out=-90,in=0] (1.5,-1.25) to[out=180,in=-55] (0,0);
			\draw[looseness=1,rotate=180, ultra thick, draw=black!30!white] (0,0) to[out=55,in=180] (1.5,1.25) to[out=0,in=90] (2.5,0) to[out=-90,in=0] (1.5,-1.25) to[out=180,in=-55] (0,0);
			\draw[->] (-3,0) -- (3,0);
			\draw[->] (0,-2) -- (0,2);
			\draw[very thick, ->] (0,0) -- node[left] {$ \vec{p} $} (0,1.5);
			\draw[->] (0,0) -- (2.75,1.5) node[anchor=south east] {$ \vec{r} $};
		\end{tikzpicture}
	\end{center}
