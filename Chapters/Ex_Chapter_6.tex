...
\begin{enumerate}[1)]
	...
	\begin{minipage}{.2\linewidth}
		...
		% t1:
		\begin{tikzpicture}
			\draw[pattern=north east lines] (0,0) circle (.3cm);
			\draw[thick,->] (0,-1.5) -- (0,1.5) node[anchor=south east] {$ \vec{\mu} $};
			\draw[->] ($ ({cos(45) * -1},{sin(45) * .7}) $) arc[start angle=-225, end angle=10, x radius = 1cm, y radius = .7cm] node[anchor=south west] {$ q $};
			\draw[dashed] ($ ({cos(45) * -1},{sin(45) * .7}) $) arc[start angle=135, end angle=10, x radius = 1cm, y radius = .7cm];
		\end{tikzpicture}
	\end{minipage}%
	\\
	...
	\lcom{Wechselwirkung zwischen dem Elektronen-Spin-Drehimpuls (oder Eigendrehimpuls) und dem Bahndrehimpuls des Elektrons.}
	\begin{align*}
...
\end{itemize}
\folie{Entartungen der Quantenzustände}\\
\folie{mehr Entartungen von QZ mit Zahlen}\\
\folie{Alkali Metalle}\\
\lcom{Raumtemperatur hat eine Energie von $ \frac{1}{40} \tx{ eV } $}

\subsection{QED Korrekturen: Lamb-Shift}

\folie{mehr Entartungen von QZ mit Zahlen}\\
,,Anschaulich``\,: Energie-Unschärferelation $ \Delta E \ \Delta t \ge \hbar $\\
\lcom{Die bisherigen QM Unschärferelationen stammen aus nicht kommutierenden Operatoren, wodurch eine Größe ,,Unscharf`` wird. Diese Unschärferelation stammt direkt aus der Unschärfe der Fouriertransformation (Bei der Ortsunschärfe gilt beides: Operatoren kommutieren nicht und Fouriertransformation).}
\begin{enumerate}[$ \Rightarrow $]
